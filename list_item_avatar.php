...
if (checkLogin()) {
	...
	$allData = getAllAvatarData($char_index);
	// Bought id
	$idList = array();
	$query_inventory = mysql_query("SELECT * FROM inventory_avatar WHERE userid='".$userid."' AND char_index='".$char_index."'");
	while ($data_inventory = mysql_fetch_assoc($query_inventory)) {
		$idList[] = $data_inventory['avatarid'];
	}
	$results = array();
	...
	for ($i = 0; $i < $allDataSize; ++$i) {
		...
		if (in_array($id, $idList)) {
			if (mysql_num_rows(mysql_query("SELECT ".$char_indexes[$char_index]." FROM usage_avatar WHERE ".$char_indexes[$char_index]."='".$id."' AND userid='".$userid."'")))
			{
